...
#include "rigel/engine/random_number_generator.hpp"
 ...
 * found at offset 0x1AF33 (registered version EXE).
 *
 * The original game's random number "generation" is in fact nothing more than
 * reading consecutive numbers from this table.
 */
...
int RandomNumberGenerator::gen() {
	static_assert(
		std::is_unsigned_v<decltype(mNextNumberIndex)>,
		"We rely on unsigned wraparound");
	static_assert(
		RANDOM_NUMBER_TABLE.size() ==
			static_cast<size_t>(
				std::numeric_limits<decltype(mNextNumberIndex)>::max()) +
				1u,
