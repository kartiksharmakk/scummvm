...
//TODO implement scummmvm derivatives to remove entityx dependency
...
struct MovingBody {
	MovingBody(
		...
		: mVelocity(velocity), mGravityAffected(gravityAffected), mIgnoreCollisions(ignoreCollisions) {
	}
...
struct MovementSequence {
	...
	explicit MovementSequence(
		...
		: mVelocites(velocities), mResetVelocityAfterSequence(resetVelocityAfterSequence), mEnableX(enableX) {
	}
