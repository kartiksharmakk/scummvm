...
#include "rigel/ui/text_entry_widget.hpp"
...
private:
	...
	struct ScriptedMenu {
		...
		ScriptedMenu(
			...
			: mScriptFinishedHook(std::forward<ScriptEndHook>(scriptEndHook)), mEventHook(std::forward<EventHook>(eventHook)), mpScriptRunner(pScriptRunner), mIsTransparent(isTransparent) {
		}
		#endif
		void handleEvent(const SDL_Event &event);
		void updateAndRender(engine::TimeDelta dt);
