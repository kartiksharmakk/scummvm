...
struct DrawSprite {
	DrawSprite(int x_, int y_, int spriteId_, int frameNumber_)
		: x(x_), y(y_), spriteId(spriteId_), frameNumber(frameNumber_) {
	}
...
struct DrawText {
	...
	DrawText(int x_, int y_, TextT &&text_)
		: x(x_), y(y_), text(std::forward<TextT>(text_)) {
	}
...
struct DrawBigText {
	...
	DrawBigText(const int x_, const int y_, const int colorIndex_, TextT &&text_)
		: x(x_), y(y_), colorIndex(colorIndex_), text(std::forward<TextT>(text_)) {
	}
...
struct SetupCheckBoxes {
	...
	SetupCheckBoxes(const int xPos_, DefinitionListT &&boxDefinitions_)
		: xPos(xPos_), boxDefinitions(std::forward<DefinitionListT>(boxDefinitions_)) {
	}
