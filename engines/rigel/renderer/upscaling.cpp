...
UpscalingBuffer::UpscalingBuffer(
	Renderer *pRenderer,
	const data::GameOptions &options)
	: mRenderTarget(renderer::createFullscreenRenderTarget(pRenderer, options)), mSharpBilinearShader(SHARP_BILINEAR_SHADER), mpRenderer(pRenderer), mFilter(options.mUpscalingFilter) {
}
