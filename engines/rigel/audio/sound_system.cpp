...
#include "rigel/audio/sound_system.hpp"

#include "rigel/assets/audio_package.hpp"
#include "rigel/assets/resource_loader.hpp"
#include "rigel/audio/adlib_emulator.hpp"
#include "rigel/audio/software_imf_player.hpp"
...
struct SoundSystem::ImfPlayerWrapper {
	ImfPlayerWrapper(
		...
		const int sampleRate,
		const int numChannels)
		: mPlayer(sampleRate), mBytesPerSample((SDL_AUDIO_BITSIZE(audioFormat) / 8) * numChannels) {
		SDL_BuildAudioCVT(
			&mConversionSpecs,
			AUDIO_S16LSB,
			1,
...
SoundSystem::LoadedSound::LoadedSound(RawBuffer buffer)
	: mData(std::move(buffer)), mpMixChunk(sdl_utils::wrap(
...
SoundSystem::SoundSystem(
	...
	: mCloseMixerGuard(std::invoke([]() {
	  ...
	  mpResources(pResources), mCurrentSoundStyle(soundStyle), mCurrentAdlibPlaybackType(adlibPlaybackType) {
	int sampleRate = 0;
